...
input <- "merged_dna_rna.txt"
df <- read_tsv(input)
chromColor <- read_tsv("./chromColor.txt")
dd <- df %>% filter(QUAL > 300)  # 过滤QUAL低于300的位点

l1<-nchar(dd$REF)	#求REF字符串长度，nchar的参数是一个字符向量，返回对应位置上字符串的长度
...
hh <- hh %>% mutate(WT.index = WT.alt.depth/(WT.ref.depth + WT.alt.depth),
                    mut.index = mut.alt.depth/(mut.ref.depth + mut.alt.depth),
                    delta_index = WT.index - mut.index)
...
w <- winScan(x = hh,
             ...
             position = "POS",
             values = c("ED", "ED4","WT.index", "mut.index","delta_index"), 
             win_size = 1000000,
             win_step = 100000,
             funs = c("mean"))  # funs = c("mean"): 这是你要应用于每个窗口的统计函数，这里指定了 "mean"，表示计算每个窗口中的平均值。

w <- w %>% select(CHROM, win_start, win_end, win_mid, ED = ED_mean, ED4 = ED4_mean, SNP_number = ED_n, WT_index = WT.index_mean, mut_index = mut.index_mean, delta_index = delta_index_mean)
write_tsv(file = "BSR_ED_ED4_SNPindex.txt", x = w)
...
EDh1_filter_data <- d%>% filter(ED >= EDh1)
...
# SNP-index
WT_index1 = quantile(na.omit(w$WT_index), probs = seq(0,1,0.01))['99%']
WT_index2 = quantile(na.omit(w$WT_index), probs = seq(0,1,0.01))['95%']

WT_index1_filter_data <- d %>% filter(WT_index >= WT_index1)
write_tsv(file="WT_index_99_filter.txt", x=WT_index1_filter_data)

WT_index2_filter_data <- d %>% filter(WT_index >= WT_index2)
write_tsv(file="WT_index_95_filter.txt", x=WT_index2_filter_data)

mut_index1 = quantile(na.omit(w$mut_index), probs = seq(0,1,0.01))['99%']
mut_index2 = quantile(na.omit(w$mut_index), probs = seq(0,1,0.01))['95%']

mut_index1_filter_data <- d %>% filter(mut_index >= mut_index1)
write_tsv(file="mut_index_99_filter.txt", x=mut_index1_filter_data)

mut_index2_filter_data <- d %>% filter(mut_index >= mut_index2)
write_tsv(file="mut_index_95_filter.txt", x=mut_index2_filter_data)

delta_index1 = quantile(na.omit(w$delta_index), probs = seq(0,1,0.01))['99%']
delta_index2 = quantile(na.omit(w$delta_index), probs = seq(0,1,0.01))['95%']

delta_index1_filter_data <- d %>% filter(delta_index >= delta_index1)
write_tsv(file="delta_index_99_filter.txt", x=delta_index1_filter_data)

delta_index2_filter_data <- d %>% filter(delta_index >= delta_index2)
...
delta_index1_neg_filter_data <- d %>% filter(delta_index <= delta_index1_neg)
write_tsv(file="delta_index_99_neg_filter.txt", x=delta_index1_neg_filter_data)

delta_index2_neg_filter_data <- d %>% filter(delta_index <= delta_index2_neg)
...
# SNP-index-WT
P1 <- ggplot(na.omit(hh), aes(x = POS, y = WT.index)) +
  geom_point(aes(color = COLOR), size = 1.5,stroke=0,alpha=0.9) +scale_color_identity() +
  labs(x="", y="WT_index") +
  scale_x_continuous(breaks = NULL, expand = c(0, 0)) +
  theme_cowplot() +
  ...
  facet_grid(. ~ LABEL, scales = "free_x", space = "free_x")
#添加window的平均值的折线图
P2 = P1+geom_line(data=filter(d, SNP_number > 10),aes(x = win_mid, y = WT_index, group=LABEL),color = "black", size = 0.7)
#添加阈值线
...
P3
ggsave(P3, filename = "SWL6_WT_index.pdf", width = 15, height = 4)


# SNP-index-mut
P1 <- ggplot(na.omit(hh), aes(x = POS, y = mut.index)) +
  geom_point(aes(color = COLOR), size = 1.5,stroke=0,alpha=0.9) +scale_color_identity() +
  labs(x="", y="mut_index") +
  scale_x_continuous(breaks = NULL, expand = c(0, 0)) +
  theme_cowplot() +
  ...
  facet_grid(. ~ LABEL, scales = "free_x", space = "free_x")
#添加window的平均值的折线图
P2 = P1+geom_line(data=filter(d, SNP_number > 10),aes(x = win_mid, y = mut_index, group=LABEL),color = "black", size = 0.7)
#添加阈值线
...
# delta-index
P1 <- ggplot(na.omit(hh), aes(x = POS, y = delta_index)) +
  geom_point(aes(color = COLOR), size = 1.5,stroke=0,alpha=0.9) +scale_color_identity() +
  ...
  facet_grid(. ~ LABEL, scales = "free_x", space = "free_x")
#添加window的平均值的折线图
P2 = P1+geom_line(data=filter(d, SNP_number > 10),aes(x = win_mid, y = delta_index, group=LABEL),color = "black", size = 0.7)
#添加阈值线
...
P3
ggsave(P3, filename = "SWL6_delta_index.pdf", width = 15, height = 4)
